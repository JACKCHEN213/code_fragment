...
if __name__ == '__main__':
    ...
    model_file = '.\\src\\adult.pkl'
    if os.path.exists(model_file):
        model = joblib.load(model_file)
    else:
        show_result = True
        print('读入数据')
        data = pd.read_csv('src\\adult.data', header=None, names=column_names)
        # print data
    ...
    data_test = pd.read_csv('src\\adult.test', header=None, skiprows=1, names=column_names)
