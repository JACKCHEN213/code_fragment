...
def save_result(model):
    ...
    data_test.to_csv('src\\Prediction.csv', header=True, index=True, columns=['Label'])
...
if __name__ == "__main__":
    ...
    print('载入训练数据...')
    t = time()
    data = pd.read_csv('src\\MNIST.train.csv', header=0, dtype=np.int)
    ...
    print('载入测试数据...')
    t = time()
    data_test = pd.read_csv('src\\MNIST.test.csv', header=0, dtype=np.int)
    print('载入完成，耗时%f秒' % (time() - t))
    ...
    t = time() - t
    print('%s训练集准确率：%.3f%%，耗时%d分钟%.3f秒' % (
    classifier_type, accuracy_score(y_train, y_train_pred) * 100, int(t / 60), t - 60 * int(t / 60)))
    t = time()
    y_valid_pred = model.predict(x_valid)
    t = time() - t
    print('%s测试集准确率：%.3f%%，耗时%d分钟%.3f秒' % (
    classifier_type, accuracy_score(y_valid, y_valid_pred) * 100, int(t / 60), t - 60 * int(t / 60)))
    save_result(model)
